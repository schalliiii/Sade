...
let turn: number; // Speichert in welchem Zug / welcher Runde wir uns befinden
let good: boolean; // boolean Wert, true wenn der User die richtigen Töne wiederholt hat, false wenn nicht
let compTurn: boolean; // boolean Wert, speichert wer dran ist mit Töne spielen (true = computer, false = user)
let intervalId: any; // Hier wird eine Funktion gespeichert
let noise: boolean = true; // Gibt an. ob ein Sound gemacht werden soll (boolean)
let on: boolean = true; // ist Programm an?
let win: boolean; // hat der User schon gewonnen?
let level: number; // gibt die Anzahl der Sounds an, die wiederholt werden müssen zum Gewinnen
...
const button1: HTMLElement = document.querySelector("#button1"); // speichert den boobutton
const button2: HTMLElement = document.querySelector("#button2"); // speichert den laughbutton
const button3: HTMLElement = document.querySelector("#button3"); // speichert den duckbutton
const button4: HTMLElement = document.querySelector("#button4"); // speichert den boingbutton
const button5: HTMLElement = document.querySelector("#button5"); // speichert den applausebutton
...
const soundButtons: Array<HTMLElement> = [button1, button2, button3, button4, button5];
...
function play() { // funktion zum starten des Spiels
  playing = true;
...
function gameTurn() { // funktion zum ausführen eines einzelnen Spielzugs des pcs
  ...
  if (compTurn) { // Prüfen, ob der PC am Zug ist. Falls ja:              Anmerkung: Diese Bedinung ist immer wahr, wenn die Prüfung oben drüber falsch ist
    setTimeout(() => { // die setTimeout Funktion ist ebenfalls von Typescript vordefiniert und funktioniert analog zur setInterval Methode, d.h sie erwartet zwei Argumente: Code/Funktion zum Ausführen und eine Zeit. Sie führt den Code/die Funktion nach der gegebenen Zeit in Milisekunden aus, Die nächsten fünf Zeilen werden also ausgeführt, nachdem 500 ms gewartet wurde
      if (order[flash] == 1) one(); // prüft, ob das Element aus dem Order array(Erinnerung: speichert die vorgegebenen Sounds) eine 1 ist  (erinnerung: 1 ist der boo sound) Falls ja: ruft die Funktion one auf (diese spielt den boo sound, genaue funktionsweise wird später erklärt) falls nein: es geschieht nichts 
      if (order[flash] == 2) two(); //analog
      if (order[flash] == 3) three(); //analog
      if (order[flash] == 4) four(); //analog
      if (order[flash] == 5) five(); //analog
...
function one() { // Funktion zum abspielen des boo sounds
...
button1.addEventListener('click', (event) => {  // wir registrieren auf das click event des boo buttons folgenden code 
...
button2.addEventListener('click', (event) => { //analog
  if (on && playing) {
    playerOrder.push(2);
    check();
    two();
...
button3.addEventListener('click', (event) => { //analog
  if (on && playing) {
    playerOrder.push(3);
    check();
    three();
...
button4.addEventListener('click', (event) => { //analog
  if (on && playing) {
    playerOrder.push(4);
    check();
    four();
...
button5.addEventListener('click', (event) =>  {//analog
  if (on && playing) {
    playerOrder.push(5);
    check();
    five();
...
function check() { // funktion zum prüfen, ob der User die korrekten Sounds ausgewählt hat und ob das Spiel vorbei ist
...
function winGame() { // diese funktion wird aufgerufen, wenn der spieler gewonnen hat
...
function playSample(number: number) { // spielt sounds ab, bekommt eine nummer (welche wir aus dem playerOrder und dem order array nehmen) als argument, damit sie weiß, welchen sie abspielen soll
  ...
  switch(number){ // switch statement, bekommt die nummer, die die funktion als argument bekommen hat ebenfalls als argument. je nach übergebener nummer wird unterschiedlicher code ausgeführt. bekommt die funktion also zb die 1 als argument, wird der code bei case 1 ausgeführt.
    ...
    case 2: // analog
        ...
        sound = new Audio("sound_laugh.mp3"); // analog
        break; // analog
    case 3: // analog
        button3.classList.add("highlight");
        sound = new Audio("sound_squeaking.mp3"); // analog
        break; // analog
    case 4: // analog
        button4.classList.add("highlight");
        sound = new Audio("sound_boing.mp3"); // analog
        break; // analog
    case 5: // analog
        button5.classList.add("highlight");
        sound = new Audio("sound_yeah.mp3"); // analog
        break; // analog
    case 6:
...
function removeElement(elementId: any) { //entfernt das html element mit der id, die als argument übergeben wird. Diese funktion dient zum entfernen der level buttons
...
function addElement() {
  ...
  one.src ="severity1.png";
  two.src= "severity2.png";
  three.src="severity3.png";
  four.src="severity4.png";
